...
//if(isset($_GET['user']) && intval($_GET['user'])) {
 ...
 // mysql_select_db('jsontest',$link) or die('Cannot select the DB');
    ...
    $query = "";
	$query .= "SELECT wp_posts.ID event_id, CONVERT(CAST(wp_posts.post_title as BINARY) USING latin1) as event_title, 
            ...
            FROM wp_icl_translations JOIN wp_posts
	...
	if($format == 'json') {
		...
		echo json_encode(array("status" => "1",'events'=>$posts),true);
	}
	else {
