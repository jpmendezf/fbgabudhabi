...
<div id="content" <?php Avada()->layout->add_class( 'content_class' ); ?> <?php Avada()->layout->add_style( 'content_style' ); ?>>
	...
	<?php if ( ( Avada()->settings->get( 'portfolio_pn_nav' ) && 'no' != get_post_meta( $post->ID, 'pyre_post_pagination', true ) ) || ( ! Avada()->settings->get( 'portfolio_pn_nav' ) && 'yes' == get_post_meta( $post->ID, 'pyre_post_pagination', true ) ) ) : ?>
        ...
        <div style="float:left;"><h4 style="margin-top:0px;"><?php esc_html_e( 'Categories : ', 'Avada' ) ;
        echo get_the_term_list( $post->ID, 'portfolio_category', '', '', '' );?></h4></div>
        <?php endif; 
	...
	<?php if ( have_posts() ) :  the_post(); ?>
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			...
			<div class="project-content clearfix">
				...
				<div style="width:50%;padding:10px;" class="project-description post-content<?php echo ( $project_details ) ? ' fusion-project-description-details' : ''; ?>" style="<?php echo $project_desc_width_style; ?>">
					<?php if ( ! post_password_required( $post->ID ) ) : ?>
				...
				<?php if ( ! post_password_required( $post->ID ) && $project_details ) : ?>
					<div class="project-info" style="width:50%;padding:10px;"><?
                        //if(false){

						    ?><h3><?php esc_html_e( 'Privilege Details', 'Avada' ); ?></h3><?
                        //}
