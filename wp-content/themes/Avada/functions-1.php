...
function create_individuall( $entry, $form ) {
	...
	$result_v = $wpdb->get_results("SELECT * FROM wp_usermeta WHERE meta_key='contact_id' AND meta_value='{$contact_id}'");
...
function suggestion_profile($type="list") {
	...
	foreach ($result as $row) {
		...
		$approved=false;
		$result9 = $wpdb->get_results(" SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$row->ID}' AND meta_key='pw_user_status' AND meta_value='approved' ");	
		...
		$result22 = $wpdb->get_results(" SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$row->ID}' AND meta_key='paid_unpaid' AND meta_value='paid' ");	
		...
		if ($current_user_id<>"") {
			...
			$result70 = $wpdb->get_results(" SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$current_user_id}' AND meta_key='paid_unpaid' AND meta_value='paid'");
			...
			$result377 = $wpdb->get_results(" SELECT `user_id` FROM wp_usermeta WHERE user_id='{$row->ID}' AND meta_key='members_overview' AND meta_value='a:1:{i:0;s:7:\"Display\";}' ");
			if(empty($result377)) {
				continue;
			}
		...
		$first_name="";
		$result1 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='first_name'");
		foreach ($result1 as $row1) {
			$first_name=$row1->meta_value;
		}

		$last_name="";
		$result3 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='last_name'");	
		foreach ($result3 as $row3) {
			$last_name=$row3->meta_value;
		}

		$badge_name="";
		$result4 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='badge_name'");	
		foreach ($result4 as $row4) {
			$badge_name=$row4->meta_value;
		}

		$result8 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='badge_color'");	
		foreach ($result8 as $row8) {
			$badge_color=$row8->meta_value;
		}

		$result9 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='badge_background'");	
		foreach ($result9 as $row9) {
			$badge_background=$row9->meta_value;
		}

		$user_picture="";
		$result5 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='user_picture'");	
		foreach ($result5 as $row5) {
			$user_picture=$row5->meta_value;
		}

		$user_image=content_url()."/uploads/2017/01/new-FBG-logo_transparent.png";
		$result2 = $wpdb->get_results("SELECT `meta_value` FROM wp_postmeta WHERE post_id='{$user_picture}' AND meta_key='_wp_attached_file'");	
		...
		$position = "";
		$resPosition = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='position'");	
...
EOF;
        ...
        if(ICL_LANGUAGE_CODE=='fr') {
             $link_used = get_site_url() . "/fr/annuaire-en-ligne/users-details/?uid=" . $user_id;
        }
        else{
		    $link_used = get_site_url() . "/online-directory/users-details/?uid=" . $user_id;
        }
		$final_link=$company_name_c=$company_name_wm="";
		$company_id_c=0;


		$result15 = $wpdb->get_results("SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$user_id}' AND meta_key='company'");	
		...
		$result16 = $wpdb->get_results("SELECT `post_title` FROM wp_posts WHERE ID='{$company_id_c}'");	
...
function suggested_profiles_slider($atts = []) {
	...
	// $current_user_id=get_current_user_id();
	// $result77 = $wpdb->get_results(" SELECT `meta_value` FROM wp_usermeta WHERE user_id='{$current_user_id}' AND meta_key='paid_unpaid' AND meta_value='paid' ");
