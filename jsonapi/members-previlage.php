...
//if(isset($_GET['user']) && intval($_GET['user'])) {
 ...
 // mysql_select_db('jsontest',$link) or die('Cannot select the DB');
    ...
    $query = "";
	$query .= "SELECT wp_xpksy4skky_posts.ID privilege_id, CONVERT(CAST(wp_xpksy4skky_posts.post_title as BINARY) USING latin1) as privilege_title, 
            
            privilege_image.meta_value AS privilege_image
            FROM wp_icl_translations JOIN wp_xpksy4skky_posts
            ...
            WHERE wp_xpksy4skky_posts.post_status = 'publish'
             ...
             AND wp_icl_translations.element_id = wp_xpksy4skky_posts.ID
	...
	if(mysql_num_rows($result)) {
      // $posts[] =  array("success" => "1");
		while($post = mysql_fetch_assoc($result)) {
            ...
            $category = get_the_category_list( ",", "multiple", $post['privilege_id'] );
             $post['privilege_category'] = $category;


           // array_push($post, $imgName);
            $posts[] = array('privilege'=>$post);
	...
	if($format == 'json') {
		...
		sendResponse(200, json_encode(array("status" => "1",'privileges'=>$posts),true));
	}
	else {
